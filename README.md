...
**Live demo:** https://multi-agent-travel-planner-qp8l.onrender.com/
...
| | |
|---|---|
| **Input** | A free-text travel request, e.g. *"Plan a complete 7 day Japan trip from Chennai under 2 lakhs"* |
...
| **LLM provider** | OpenAI (Groq fallback) |
...
- [LangChain](https://www.langchain.com/) / `langchain-openai` — LLM message plumbing
- [OpenAI](https://openai.com/) (default model `gpt-4.1-mini`, set `OPENAI_MODEL` to change) — the LLM behind the itinerary & report agents; Groq is used only when `OPENAI_API_KEY` is unset
...
| Itinerary Agent | `itinerary_agent` | Sends flight + hotel results to the LLM to draft a day-by-day itinerary |
| Final Report Agent | `final_agent` | Sends everything to the LLM again to produce the final, formatted trip summary |
...
3. The graph runs its four nodes in order, calling AviationStack, Tavily, and the LLM along the way.
...
- API keys: [OpenAI](https://platform.openai.com/), [Tavily](https://tavily.com/), [AviationStack](https://aviationstack.com/)
...
| `OPENAI_API_KEY` | ✅ | OpenAI API key — powers the itinerary and final report agents (default provider) |
| `OPENAI_MODEL` | optional | OpenAI model name (default `gpt-4.1-mini`) |
| `GROQ_API_KEY` | optional | Groq fallback — used only when `OPENAI_API_KEY` is unset |
...
```json
{
  "message": "Plan a 7 day Japan trip from Chennai under 2 lakhs",
  "thread_id": null
}
```
